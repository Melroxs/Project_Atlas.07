...
const statusLabel = (s, map = CLAIM_STATUS_MAP) => map[s?.toLowerCase()] || (s ? String(s).replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : 'In Progress')
const rel = (d) => { try { return d ? new Date(d).toLocaleDateString() : '—' } catch { return '—' } }
...
const ClaimDetail = ({ id, onBack }) => {
  const [tab, setTab] = useState('Overview')
  const [data, setData] = useState(null)
  useEffect(() => {
    if (!id) return
    atlas.claims.get(id).then(setData).catch(() => setData(null))
  }, [id])
  const tabs = ['Overview','Timeline','Documents','Supplements','Photos','Notes']
  const c = data || {}
  const claimNumber = c.claimNumber || id
  const carrier = c.insuranceCompany || 'Unknown'
  const status = statusLabel(c.status)
  const address = [c.propertyAddress, c.propertyCity, c.propertyState].filter(Boolean).join(', ') || '—'
  const supplement = Number(c.approvedValue) || 0
  const revenue = Number(c.estimatedValue) || 0
  return (
    <>
      <DetailHeader
        eyebrow={`Claim · ${claimNumber}`}
        title={c.description || 'Residential Damage'}
        meta={<>{carrier} · Filed {rel(c.dateOfLoss || c.createdAt)} · <span className="text-white/70">{address}</span></>}
        onBack={onBack}
        right={<>
